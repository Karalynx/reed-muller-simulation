...
impl View for TextAreaV2 {
    ...
    fn on_event(&mut self, event: Event) -> EventResult {
        ...
        match event {
            Event::Char(ch) => return EventResult::Consumed(Some(self.insert(ch))),
            Event::Key(Key::Enter) => return EventResult::Consumed(Some(self.insert('\n'))),
            Event::Key(Key::Backspace) if self.cursor > 0 => return EventResult::Consumed(Some(self.backspace())),
            Event::Key(Key::Del) if self.cursor < self.content.len() => return EventResult::Consumed(Some(self.delete())),
